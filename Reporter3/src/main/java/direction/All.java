 ...
 */
@WebServlet(name = "AllD", urlPatterns = { "/Direction/All" })
public class All extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if (!Checker.usercheck(request, 1)) {
			response.sendRedirect("/Reporter/Login");
		} else {
		Database.Connect();
		int weekOfYear=0;
		...
		System.out.println("directeur id : "+utilisateur.getId());
		ArrayList<Action>actions = actiondao.getAllFromDirecteurId(utilisateur.getId());
		request.setAttribute("actions", actions);
		request.setAttribute("year", year);
		request.setAttribute("weeknumber", weekOfYear);
		request.getRequestDispatcher( "/Direction/all.jsp" ).forward( request, response );}
	}
